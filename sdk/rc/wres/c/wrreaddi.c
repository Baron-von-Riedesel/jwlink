...
/******************************************************************************/
{
    ...
    WResID                 *name;
    WResID                 *type;
    WResLangType           lang; /* jwlink */

	DEBUG(( DBG_OLD, "readMResDir: enter" ));
    error = FALSE;
    if( iswin32 ) {
        /* Read NULL header */
    ...
    while( head != NULL && !( iswin32 && head32 == NULL ) && !error ) {
        ...
        /* jwlink */
		lang.lang = head->LanguageId;
		lang.sublang = 0;

		DEBUG(( DBG_OLD, "readMResDir: name=%h type=%h lang=%h.%h", name->ID.Num, type->ID.Num, lang.lang, lang.sublang ));
        ...
        if( !error ) {
            if( !type->IsName && type->ID.Num == RT_NAMETABLE ) {
				...
				/* jwlink: WResLangType lang added */
                error = WResAddResource2( type, name, head->MemoryFlags,
                            WRESTELL( handle ), head->Size, currdir, &lang,
                            &dup, fileinfo );
	...
	DEBUG(( DBG_OLD, "readMResDir: exit" ));
    return( error );
                 ...
                 int *dup_discarded, void *fileinfo )
{
	...
	DEBUG(( DBG_OLD, "WResReadDir2: enter" ));
	...
	DEBUG(( DBG_OLD, "WResReadDir2: exit" ));
    return( error );
