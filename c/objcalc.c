...
// together.
{
    ...
    DEBUG(( DBG_OLD, "ReOrderClasses enter" ));
    ...
    for( currcl = sec->classlist; currcl != NULL; currcl = nextcl ) {
        ...
        if( ( FmtData.type & ( MK_NOVELL | MK_PHAR_LAP | MK_OS2_LX ) )
            ...
            if( currcl->flags & CLASS_CODE ) {
                ...
                || ( currcl->segs->group == NULL && !( FmtData.type & (MK_PE|MK_ELF) ) )
        ...
        DEBUG(( DBG_OLD, "ReOrderClasses: %s order=%h", currcl->name, ord ));
...
// information is provided.
{
    ...
    DEBUG(( DBG_OLD, "SortClasses enter" ));
    for( currcl = sec->classlist; currcl != NULL; currcl = nextcl ) {
        nextcl = currcl->next_class;  // Take class out of original ring
        currcl->next_class = NULL;
        CheckClassUninitialized( currcl );
...
// reorder segments based on COFF name grouping
{
    ...
    DEBUG(( DBG_OLD, "SortSegments() exit" ));
}
...
/*****************************************/
{
    ...
    if( seg->group != NULL ) {
        DEBUG(( DBG_OLD, "CalcInitSize(%s)", seg->segname ));
...
/* Calculate the starting address in the file of each segment. */
{
    ...
    if( FmtData.base == NO_BASE_SPEC ) {
        if( FmtData.type & MK_PE ) {
            ...
            } else if( FmtData.u.elf.elf64 ) { /* jwlink */
                FmtData.base = 0x00400000;
            } else {
    ...
    if( FmtData.type & ( MK_REAL_MODE | MK_FLAT | MK_ID_SPLIT ) ) {
        ...
        for( grp = Groups; grp != NULL; grp = grp->next_group ) {
			...
			/* jwlink: for ELF, objalign + segment alignment is to be used */
			if ( FmtData.type & MK_ELF ) {
                unsigned alignment;
				/* we need the group alignment (=largest segment alignment).
				 */
				if ( grp->alignment > (1 << FmtData.u.elf.segment_shift) ) {
					alignment = grp->alignment;
				} else
					alignment = 1 << FmtData.u.elf.segment_shift;
				flat = ROUND_UP( flat, alignment );
			}
            ...
            DEBUG(( DBG_OLD, "CalcAddresses(): group %s base=%h size=%h (org. size=%h)", grp->sym->name, flat, size, grp->totalsize ));
			if ( FmtData.type & MK_ELF ) {
				flat += size;
				flat += FmtData.objalign;
			} else
    ...
    DefinePublics();
    DEBUG(( DBG_OLD, "CalcAddresses() exit" ));
}
 ...
 * segments (groups) are. */
{
    ...
    for( currgrp = Groups; currgrp != NULL; currgrp = currgrp->next_group ){
        ...
        if( currgrp->totalsize != 0 ) {
            ...
            NumGroups--;
            DEBUG(( DBG_OLD, "ReallocFileSegs(): NumGroups decremented to %d, totalsize=0, grp=%s!!!!", NumGroups, currgrp->sym->name ));
        }
    }
 ...
 * DGROUP. So - this finds the start of the uninitialized portion of DGROUP */
{
    ...
    FmtData.bsspad = 0;
	/* jwlink: do it always for PE and ELF
	 * the better strategy probably is to NOT include bss sections
     * into DGROUP at all if PE|ELF is the target.
	 */
    if( FmtData.type & ( MK_PE|MK_ELF) )
        ;
    else
    ...
    if( setnext ) {             //last one was LXDATA or no segs.
        FmtData.dgroupsplitseg = NULL;
    }
}
 ...
 * not useful for OS/2 16-bit mode. */
{
    ...
    DEBUG(( DBG_OLD, "CalcGrpAddr(%s) enter", currgrp ? currgrp->sym->name : "NULL" ));
    for( ; currgrp != NULL; currgrp = currgrp->next_group ) {
        ...
        DEBUG(( DBG_OLD, "CalcGrpAddr(%s): grp_addr=%h:%h totalsize=%h",
               currgrp->sym->name, currgrp->grp_addr.seg, currgrp->grp_addr.off, currgrp->totalsize ));
    }
}
...
/* Allocate a segment (process all segments in a given class) */
{
    ...
    if( !( seg->info & SEG_ABSOLUTE ) ) {
        ...
        CurrentSeg = seg;
        NewSegment( seg );
        DEBUG(( DBG_OLD, "AllocSeg(%s): segment allocated", seg->segname ));
    }
}
...
/**********************************************/
{
    ...
    DEBUG(( DBG_OLD, "SetSegFlags() enter" ));
    for( class = Root->classlist; class != NULL; class = class->next_class ) {
    ...
    for( ; flag_list != NULL; flag_list = flag_list->next ) {
        if( ( flag_list->type == SEGFLAG_CODE )
            ...
            DEBUG(( DBG_OLD, "SetSegFlags(): TYPE flag" ));
    ...
    for( flag_list = start; flag_list != NULL; flag_list = flag_list->next ) {
        if( flag_list->type == SEGFLAG_CLASS ) {
            DEBUG(( DBG_OLD, "SetSegFlags(): CLASS flag" ));
    ...
    for( flag_list = start; flag_list != NULL; flag_list = next_one ) {
        if( flag_list->type == SEGFLAG_SEGMENT ) {
            DEBUG(( DBG_OLD, "SetSegFlags(): SEGMENT %s, flags [%x]", flag_list->name, flag_list->flags ));
            leader = FindSegment( Root, flag_list->name );
            if( leader == NULL ) {
                LnkMsg( WRN + MSG_SEG_NAME_NOT_FOUND, "s", flag_list->name );
            } else {
                DEBUG(( DBG_OLD, "SetSegFlags(): SEGMENT found, old leader flags=%x", leader->segflags ));
