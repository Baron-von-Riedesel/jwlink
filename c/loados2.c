...
/* copy code from extra memory to loadfile. */
{
    ...
    DEBUG(( DBG_OLD, "WriteOS2Data() enter" ));
...
/**********************/
{
	DEBUG(( DBG_OLD, "ChkOS2Data(): calling SetSegFlags() with FmtData.u.os2.os2_seg_flags" ));
...
/* jwlink: enable the 32-bit segment flag.
 * be very careful with function CheckGrpFlags() - it's called for 16- and 32-bit code,
 * for OS/2, Win16 and Win32 via ChkOS2Exports()!
 */
#define DEFW16_SEG_OFF (SEG_PRELOAD|SEG_INVALID|SEG_32_BIT)
...
/****************************************/
{
    ...
    if( FmtData.type & MK_WIN_NE ) {
        if ( leader->info & USE_32 )
            sflags |= SEG_32_BIT;
		if ( leader->info & SEG_CODE )
            leader->group->segflags &= ~SEG_DATA;
        // if any of these flags are on, turn it on for the entire group.
		leader->group->segflags |= sflags & DEFW16_SEG_OFF;
        /* if code segment, make everything code */
		DEBUG(( DBG_OLD, "CheckGrpFlags(): 1 grp.segflags=%h", leader->group->segflags ));
    } else {
        // if any of these flags are on, turn it on for the entire group.
	...
	DEBUG(( DBG_OLD, "CheckGrpFlags(): exit  grp.segflags=%h sflags=%h", leader->group->segflags, sflags ));
}
...
// NOTE: there is a continue in this loop!
{
    ...
    DEBUG(( DBG_OLD, "ChkOS2Exports(): enter" ));
        ...
        if( !(symptr->info & SYM_DEFINED) ) {
            ...
            if( symptr->p.seg == NULL || IS_SYM_IMPORTED(symptr) ) {
                if( FmtData.type & MK_OS2_FLAT ) {
                    ...
                    DEBUG(( DBG_OLD, "ChkOS2Exports(): invalid export %s seg=%h info=%h", symptr->name, symptr->p.seg, symptr->info ));
    ...
    DEBUG(( DBG_OLD, "ChkOS2Exports(): exit" ));
}
 ...
 * This is called for all 16-bit NE binaries (OS/2 and Windows!)
 */
{
    ...
    DEBUG(( DBG_OLD, "FiniOS2LoadFile() enter" ));
     ...
     */
    if( FmtData.u.os2.segment_shift == -1 ) {
    ...
    DEBUG(( DBG_OLD, "FiniOS2LoadFile(): WriteOS2Data" ));
    WriteOS2Data( stub_len, &exe_head );
    DEBUG(( DBG_OLD, "FiniOS2LoadFile(): WriteOS2Resources" ));
    ...
    exe_head.chk_sum = 0L;
    exe_head.info = 0;
    if ( FmtData.u.os2.dpmi16 ) {    /* jwlink */
        exe_head.target = TARGET_DPMI;
    } else if( FmtData.type & MK_WIN_NE ) {
