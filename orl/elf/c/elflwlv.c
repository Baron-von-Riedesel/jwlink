...
static orl_return ParseLnkCmd( char *cmd, char **contents, int *len,
    orl_return  (*callback_fn)( char *, char *, void * ), void *cookie )
/**************************************************************/
{
    char       *arg;
    char       *value;
    ...
    for( value = *contents, delim = 0; *len; value++, *len -= 1 ) {
        if ( delim ) {
            if ( *value == delim && ( *len == 1 || *(value+1) != delim ) )
                delim = 0;
        } else {
            if ( *value == '"' || *value == '\'' )
                delim = *value;
    ...
    *contents = value;
    if ( delim )
        return( ORL_ERROR ); /* ending quote missing */
    do {
        arg[l] = 0;
        l--;
    } while ( l >= 0 && arg[l] == ' ' || arg[l] == '\t' );
    return( callback_fn( cmd, arg, cookie ) );
}
...
/**************************************************************************/
{
    ...
    while( len > 0 ) {
        ...
        contents++; len--;
        for( cmd = lnkcmd; len && cmd < (lnkcmd+sizeof(lnkcmd)-1) && isalnum( *contents ); cmd++, contents++, len-- )
            *cmd = tolower(*contents);
        *cmd = '\0';
        if ( *contents == ':' && len ) {
            contents++;
            len--;
        }
        /* jwlink: let the linker decide what cmd it will accept! */
        if( ParseLnkCmd( lnkcmd, &contents, &len, cb->lnk_cmd_fn, cookie ) != ORL_OKAY )
            break;
        EatWhite( &contents, &len );
    }
    return( ORL_OKAY );
